...
func writeUpstreamErrorWithAccount(w http.ResponseWriter, err error, accountID string) {
	...
	if status == http.StatusTooManyRequests {
		...
		if errors.Is(err, chathub.ErrImageLimit) {
			writeOpenAIError(w, status, "image_limit_error", "image_generation_daily_limit_reached_try_again_tomorrow", "image generation daily limit reached; try again tomorrow")
			return
		}
		writeOpenAIError(w, status, "rate_limit_error", "upstream_is_rate_limiting_try_again_shortly", "upstream is rate limiting; try again shortly")
		return
	}
	if IsEmptyCompletion(err) {
		writeOpenAIError(w, http.StatusBadGateway, "upstream_error", "upstream_returned_empty_completion_the_requested_model_may_be_unavailable_for_this_tenant", "upstream returned empty completion; the requested model may be unavailable for this tenant")
		return
	}
	if errors.Is(err, chathub.ErrOffensiveContent) {
		writeOpenAIError(w, http.StatusServiceUnavailable, "upstream_content_blocked", "m365_content_policy_blocked_this_request_try_again_or_switch_account", "M365 content policy blocked this request; try again or switch account")
		return
	}
	writeOpenAIError(w, status, "upstream_error", "upstreamerror_err", upstreamError(err))
}
