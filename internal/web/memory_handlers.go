...
func proxySubstrate(w http.ResponseWriter, targetURL string, method string, acc auth.AccountToken, body io.Reader) {
	...
	if err != nil {
		writeOpenAIError(w, http.StatusInternalServerError, "internal_error", "failed_to_create_substrate_request", "failed to create substrate request")
		return
	}
	...
	if err != nil {
		...
		writeOpenAIError(w, http.StatusBadGateway, "upstream_error", "substrate_request_failed", "substrate request failed")
		return
	}
	defer resp.Body.Close()
	respBody, err := io.ReadAll(io.LimitReader(resp.Body, 1<<20))
	if err != nil {
		writeOpenAIError(w, http.StatusBadGateway, "upstream_error", "substrate_read_failed", "substrate read failed")
		return
	}
...
func (s *Server) memoryGetFlags(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		writeOpenAIError(w, http.StatusBadGateway, "account_error", "no_account_available", "no account available")
		return
	}
	flagsCache.Lock()
...
func (s *Server) memoryPatchFlags(w http.ResponseWriter, r *http.Request) {
	if !s.validAdminSession(r) {
		writeOpenAIError(w, http.StatusForbidden, "insufficient_permission", "administrator_login_required_for_this_operation_sign_in_via_api_admin_login_and_retry_with_the_admin_session_cookie", "administrator login required for this operation; sign in via /api/admin/login and retry with the admin session cookie")
		return
	}
	acc, ok := s.memoryAccount(r)
	if !ok {
...
func (s *Server) memoryDeleteInstruction(w http.ResponseWriter, r *http.Request) {
	...
	if id == "" {
		writeOpenAIError(w, http.StatusBadRequest, "invalid_request_error", "instruction_id_required", "instruction id required")
		return
	}
